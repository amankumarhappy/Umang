...
const Cultural = () => {
    ...
    const culturalEvents = eventsData.filter(e => e.category === 'cultural');
    ...
    return (
        <div className="pt-24 min-h-screen container pb-20 px-4">
            <div className="flex flex-col md:flex-row justify-between items-center mb-8 gap-6">
                <div>
                    <h1 className="text-4xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-brand-blue to-brand-orange mb-2">Literature & Cultural</h1>
                    <p className="text-gray-600 dark:text-gray-400">Celebating creativity, art, and expression.</p>
                </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                {filteredEvents.length > 0 ? (
                    filteredEvents.map(event => (
                        <Link to={`/events/${event.id}`} key={event.id} className="group bg-white dark:bg-slate-800 rounded-2xl p-6 shadow-sm hover:shadow-xl transition-all border border-transparent hover:border-brand-blue block">
                            ...
                            <div className="flex justify-between items-center mb-4">
                                <p className="text-sm text-gray-500 dark:text-gray-400">{event.gender} Category</p>
                                {event.date && <p className="text-xs text-brand-orange font-semibold">{event.date}</p>}
                            </div>
                    ...
                    <div className="col-span-full py-20 text-center">
                        <p className="text-gray-500 dark:text-gray-400 text-lg">No cultural events found with status "{filter}".</p>
                    </div>
                )}
            </div>
        </div>
